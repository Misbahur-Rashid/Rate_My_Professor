            ...
            <div class="col-12">
                <div class="material-card card">
                    <div class="card-header">
						<div class="pull-left">
							<h6 class="card-title mt-1">Feedback Messages</h6>
						</div>
					</div>
                    <div class="card-body">
                                ...
                                <tbody>
                                    @foreach($contactmessages as $key=>$contactMessage)
                                        <tr class="{{$contactMessage->status == 0 ? 'unreaded' : ''}}">
                                            <td>{{$key + 1}}</td>
                                            <td>{{$contactMessage->teacher->name}}</td>
                                            <td>{{$contactMessage->uses[0]->name}}</td>
                                            <td>{{$contactMessage->uses[0]->phone}}</td>
                                            <td>{{$contactMessage->three_star}}</td>
                                            <td>{{str_limit($contactMessage->comment,30)}}</td>
                                            ...
                                            @endif
                                            <td>
                                                <a href="{{url('admin/feedback/'.$contactMessage->id)}}"><button class="btn btn-info btn-circle btn-sm"><i class="mdi mdi-eye"></i></button></a>
                                                <button data-url="{{url('admin/feedbacks/'.$contactMessage->id)}}" class="btn-delete btn btn-danger btn-circle btn-sm" data-toggle="modal" data-target="#deleteModal"><i class="mdi mdi-delete"></i></button>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
